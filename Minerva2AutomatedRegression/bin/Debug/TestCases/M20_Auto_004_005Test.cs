    ...
    class M20_Auto_004_005Test
    {
        public static void Execute()
        {
            ...
            try
            {
                ...
                Playback.Wait(1000);
                RecordCount = Int32.Parse(hpobj.PaginationText.InnerText.Trim().Split('-')[0].Trim());
                if (RecordCount == 1)
                {
                    Assert.Fail("Clicking on Clear Search button did not clear records");
                }
                if (RecordCount == 0)
                {
                    Console.WriteLine("Clicking on Search button has cleared records");
                }
